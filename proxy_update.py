...
"""
EV Lab Proxy Server
Combines: Lucid EVSE (10.91.0.250) + Pilot Dingus (/dev/ttyACM4)

Uses subprocess curl exclusively — bypasses OpenSSL 3.0 TLS incompatibility
with the charger's embedded server.

Run:
  pip3 install 'fastapi' 'uvicorn[standard]' pyserial --break-system-packages
...
try:
    pilot = PilotDingus(port="/dev/ttyACM4")
    PILOT_OK = True
    print("[Pilot] Pilot Dingus connected ✅")
...
def normalize_pilot(d: dict) -> dict:
    if not d:
    ...
    return {
        ...
        "mode":          d.get("mode",          "N/A"),
        "state":         d.get("state",         "U"),
        "high_v":        round(float(d.get("high_v",      0)), 2),
        "low_v":         round(float(d.get("low_v",       0)), 2),
        "duty_pct":      round(float(d.get("duty",        0)) * 100, 1),
        "frequency_hz":  round(float(d.get("frequency",   0)), 1),
        "adv_current":   round(float(d.get("adv_current", 0)), 1),
        "state_changes": d.get("state_changes", 0),
    }
...
async def on_startup():
    ...
    print(f"[Startup] Pilot    : /dev/ttyACM4")
